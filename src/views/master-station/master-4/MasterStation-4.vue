...
const equipmentRunStep = async () => {
    const response = await getRunningEquipment(masterNum);
        ...
        <el-col :span="18" :xs="24" class="full">
            ...
            <el-row class="equipment-page-h-80 p-2" id="master-div-2 ">
                <div class="base-div chart-container">
                    <div class="left p-2">
                        <CO2HistoryChart :masterNum="masterNum"/>
                    </div>
                    <div class="right p-2">
                        <APHistoryChart :masterNum="masterNum"/>
                    </div>
                </div>
            </el-row>
        </el-col>
                    ...
                    <div class="switch_group re-text" v-for="(group, index) in switchData" v-else>
                        ...
                        <div class="switchs text-center">
                            <div class="switch" v-for="item in group">
                                ...
                                <el-button size="small" type="default" round v-if="item.value"
                                           @click="switchReserve(item.value)">Close
                                </el-button>
                                <el-button size="small" type="primary" round v-else @click="switchReserve(item.value)">
                                    Open
                                </el-button>
                            </div>
                        </div>
